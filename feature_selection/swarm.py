...
class PSO:
    ...
    w: [float (0.0:1.0)] Inertia weight
    c1: [float (0.0:1.0)] Cognitive coefficient
    c2: [float (0.0:1.0)] Social coefficient
    """

    def __init__(self, dataset, target, metric, list_exp, pop, gen, w, c1, c2):

        self.heuristic = "Swarm"
    ...
    def fitness_swarm(self, pop, method):
        scores = []
        models = []
        cols = []
        scoresA = []
        ...
        scoresF = []
        for particle in pop:
            score, accuracy, precision, recall, fscore, model, col = \
                utility.fitness_ind_feature(n_class=self.n_class, d=self.data, ind=particle.position,
                                            target_name=self.target, metric=self.metric, method=method)
            if particle.pbest_value < score:
        ...
        return scores, models, cols, scoresA, scoresP, scoresR, scoresF

    def move_particles(self, pop, bestInd):
        for particle in pop:
            ...
            c = particle.c2 & (bestInd ^ particle.position)

            particle.velocity = a | b | c
            particle.move()

    def get_entropy(self, pop, inds, size):
        truth_list = []
        false_list = []
        for i in range(size):
            transpose = [row.position[i] for row in pop]
    ...
    def write_res(self, folderName, y1, y2, colMax, bestScorePro,
        ...
        string = "heuristique: Optimisation par essaim de particules" + os.linesep + \
                 ...
                 "w: " + str(self.w) + os.linesep + "c1: " + str(self.c1) + os.linesep +\
                 "c2: " + str(self.c2) + os.linesep + \
    ...
    def swarm_evolution(self, part, besties, names, iters, times, names2, features, names3):
        ...
        for mode in part:
            ...
            pop = [Particle(dimension=self.n_ind) for _ in range(self.n_pop)]

            scores, models, cols, scoresA, scoresP, scoresR, scoresF = self.fitness_swarm(pop, method)

            pop_tmp = [particle.position for particle in pop]
            ...
            bestModelPro, bestIndsPro, bestColsPro, bestAPro, bestPPro, bestRPro, bestFPro = \
                utility.feature_add_list(scores=scores, models=models, inds=pop_tmp, cols=cols, scoresA=scoresA,
            ...
            for generation in range(self.n_gen):
                ...
                self.move_particles(pop, bestInd)
                ...
                entropy = self.get_entropy(pop=pop, inds=self.n_pop, size=self.n_ind)
                ...
                x1, y1, y2, yTps, yVars = utility.add_axis_vars(bestScore=scoreMax, meanScore=mean_scores,
                                                                iter=generation,
                                                                vars=len(colMax), time_debut=time_debut, x1=x1,
                                                                y1=y1, y2=y2, yTps=yTps, yVars=yVars)

                utility.plot_feature(x1=x1, y1=y1, y2=y2, yTps=yTps, yVars=yVars, n_pop=self.n_pop, n_gen=self.n_gen,
                                     heuristic="Optimisation par essaim de particules", folderName=folderName,
    ...
    def init(self):
        ...
        pool.starmap(self.swarm_evolution, arglist)
        pool.close()
        ...
        return utility.res(heuristic="Optimisation par essaim de particules", besties=bestiesLst, names=namesLst,
...
if __name__ == '__main__':
    ...
    swarm = PSO(dataset="scene", target="Urban",
                metric="accuracy",
                list_exp=["lr", "svm", "gnb"],
                pop=30, gen=20, w=0.5, c1=0.5, c2=0.5)
