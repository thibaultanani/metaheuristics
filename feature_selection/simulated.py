...
class Simulated:
    ...
    def write_res(self, folderName, y1, y2, colMax, bestScorePro,
        ...
        f = open(a, "w")
        string = "heuristique: Recuit simulé" + os.linesep + \
    ...
    def search(self, part, besties, names, iters, times, names2, features, names3):
        ...
        for mode in part:
            ...
            score, accuracy, precision, recall, fscore, model, col = \
                utility.fitness_ind_feature(n_class=self.n_class, d=self.data, ind=solution,
            ...
            for generation in range(self.n_gen):
                ...
                neighbors_solutions = self.generate_neighbors(solution=solution, n_neighbors=self.n_pop,
                                                              n_mute_max=self.n_mut)
                ...
                x1, y1, y2, yTps, yVars = utility.add_axis_vars(bestScore=bestScore, meanScore=mean_scores,
                                                                iter=generation,
                                                                vars=sum(solution), time_debut=time_debut, x1=x1, y1=y1,
                ...
                utility.plot_feature(x1=x1, y1=y1, y2=y2, yTps=yTps, yVars=yVars, n_pop=self.n_pop, n_gen=self.n_gen,
                                     heuristic="Recuit simulé", folderName=folderName, path=self.path2,
                ...
                temperature = self.n_gen - generation

                generation = generation + 1

                cost = bestScore - res_sol
                if cost >= 0:
                    solution = bestInd
                    res_sol = bestScore
                else:
    ...
    def init(self):
        ...
        return utility.res(heuristic="Recuit simulé", besties=bestiesLst, names=namesLst, iters=itersLst,
...
if __name__ == '__main__':
    ...
    simulated = Simulated(dataset="scene", target="Urban",
                          metric="accuracy",
                          list_exp=["lr", "svm", "gnb"],
                          pop=30, gen=20, mut=5)
