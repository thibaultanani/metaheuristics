...
class Tabu:
    ...
    """
    def __init__(self, dataset, target, metric, list_exp, pop, gen, tab, mut):
    ...
    def search(self, part, besties, names, iters, times, names2, features, names3):
        ...
        for mode in part:
            ...
            for generation in range(self.n_gen):
                ...
                neighbors_solutions, tabu_list = self.generate_neighbors(solution=solution, n_neighbors=self.n_pop,
                ...
                utility.plot_feature(x1=x1, y1=y1, y2=y2, yTps=yTps, yVars=yVars, n_pop=self.n_pop, n_gen=self.n_gen,
                                     heuristic="Recherche tabou", folderName=folderName, path=self.path2,
    ...
    def init(self):
        ...
        return utility.res(heuristic="Recherche tabou", besties=bestiesLst, names=namesLst, iters=itersLst,
...
if __name__ == '__main__':
    ...
    tabou = Tabu(dataset="scene", target="Urban",
                 metric="accuracy",
                 list_exp=["lr", "svm", "gnb"],
                 pop=30, gen=20, tab=200, mut=5)
